...
from src.models.matrix_factorization.data import iter_pointwise_batches
from src.models.matrix_factorization.evaluate import evaluate_model
...
def train_one_epoch(
    ...
    if total_batches == 0:
        raise RuntimeError("Không đọc được lô dữ liệu train nào.")
...
def find_history_for_resume(
    ...
    candidates = [
        ...
        resume_checkpoint.parent / "mf_history.csv",
    ]
...
def clear_training_outputs() -> None:
    """Xóa kết quả MF cũ khi train lại từ đầu."""
...
def checkpoint_epoch(path: Path) -> int | None:
    ...
    elif stem.startswith("mf_epoch_"):
        suffix = stem.removeprefix("mf_epoch_")
    else:
        return None
...
def find_epoch_checkpoint(
    ...
    candidates = [
        ...
        CHECKPOINT_DIR / f"mf_epoch_{epoch}.pt",
        resume_checkpoint.parent / f"epoch_{epoch:03d}.pt",
        resume_checkpoint.parent / f"mf_epoch_{epoch}.pt",
    ]
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Huấn luyện Matrix Factorization trên MINDlarge."
    )
    parser.add_argument(
        "--resume",
        type=Path,
    ...
    parser.add_argument(
        "--no-bias",
        action="store_true",
        help="Tắt user bias và item bias khi train mới.",
    )
...
def main() -> None:
    ...
    print(f"Kích thước lô: {args.batch_size}")
    print(
